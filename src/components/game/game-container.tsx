...
export const GameContainer = ({ initialState }: { initialState?: GameState }) => {
  ...
  const newsItems = [
    "BOMBSHELL: Fans plan shock protest outside stadium after tactical leaks expose dressing room chaos.",
    "EXCLUSIVE: Board secretly considering managerial alternatives — sources close to chairman reveal all.",
    "BREAKING: Star striker linked with sensational summer move to bitter rivals in stunning transfer twist.",
    "SHOCK TAKEOVER: Mystery consortium circles club with jaw-dropping billion-pound bid looming.",
    "CRISIS: Scouting reports expose catastrophic lack of depth in defensive areas as season reaches boiling point.",
  ];

  // ── Season end: render SeasonSummary ────────────────────────────────────
  if (state && (state.isSeasonEnd || state.isSacked)) {
          ...
          <div className="w-full space-y-5 animate-in fade-in slide-in-from-right-4 duration-500 z-50">
            ...
            <div className="grid gap-2">
              {(Object.keys(CAREER_MODES) as CareerMode[]).map((mode) => (
                <button key={mode}
                  onClick={() => { setSetupMode(mode); setSetupStep(2); }}
            ...
            <div className="grid gap-2">
              {CAREER_MODES[setupMode].durations.map((d, idx) => (
                <button key={idx} onClick={() => setSetupDuration(idx)}
                  className={cn("flex justify-between items-center p-4 rounded border transition-all",
  ...
  return (
    <div className="flex flex-col h-screen max-md:max-w-md md:max-w-md mx-auto relative overflow-hidden bg-background shadow-2xl border-x border-white/5">
      ...
      <div className="bg-black/60 border-b border-white/5 p-2 z-[90] backdrop-blur-xl">
        ...
        <div className="flex justify-between items-center px-2 py-0.5 text-[8px] font-headline uppercase text-white border-b border-white/10 mb-1 font-black">
        ...
        <div className="flex flex-col gap-0.5">
          {windowedLeagueTable.map((team) => (
            <div key={team.team}
              className={cn("flex justify-between items-center px-2 py-1 rounded text-[10px] transition-colors",
      ...
      <div className="bg-black/95 border-t border-white/10 z-[100]">
        <div className="p-4 py-3 space-y-2">
          ...
          <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden border border-white/5">
            <div className={cn("h-full transition-all duration-1000 ease-linear", timeLeft <= 5 ? "bg-destructive shadow-[0_0_8px_rgba(239,68,68,0.5)]" : "bg-primary")}
              style={{ width: `${(timeLeft / 15) * 100}%` }} />
          </div>
        </div>

        {/* Ticker */}
        <div className="border-t border-white/5 h-10 flex items-center overflow-hidden relative"
          style={{ background: 'rgba(251,177,60,0.08)' }}>
          <div className="absolute left-0 top-0 bottom-0 z-20 flex items-center px-3 font-headline font-black text-[10px] tracking-[2.5px] uppercase"
