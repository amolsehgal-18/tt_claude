      ...
      <div className="max-w-sm mx-auto px-4 pt-3 pb-4 space-y-3">
        ...
        {!loading && (
          <div className="text-center">
            <p className="text-[9px] uppercase tracking-[0.25em]"
              style={{ fontFamily:"'Share Tech Mono',monospace", color:'rgba(255,255,255,0.22)' }}>
              Share your archetype · {archetype}
            </p>
          </div>
        )}
        ...
        <div className="space-y-2">
          ...
          <button onClick={handleShare} disabled={sharing || loading}
            ...
            {sharing ? 'Generating…' : shared ? 'Copied to Clipboard ✓' : '📸 Share Season Card'}
          </button>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <a
              href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(
                `I managed like ${archetype} — W${state.wins} D${state.draws} L${state.losses} · ${state.currentLeaguePosition}${ordinalSuffix(state.currentLeaguePosition)} place. Can you do better? 🏟️ #TouchlineTantrum`
              )}&url=${encodeURIComponent('https://touchlinetantrum.com')}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 py-2 rounded uppercase tracking-widest text-[10px] font-black transition-opacity hover:opacity-80"
              style={{ fontFamily:"'Barlow Condensed',sans-serif", background:'rgba(0,0,0,0.4)', border:'1px solid rgba(255,255,255,0.12)', color:'rgba(255,255,255,0.7)' }}
            >
            ...
            <a
              href={`https://wa.me/?text=${encodeURIComponent(
                `I just managed ${state.userTeam} as ${archetype} — ${state.wins}W ${state.draws}D ${state.losses}L, finished ${state.currentLeaguePosition}${ordinalSuffix(state.currentLeaguePosition)}. Play Touchline Tantrum: https://touchlinetantrum.com #TouchlineTantrum`
              )}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-center gap-1.5 py-2 rounded uppercase tracking-widest text-[10px] font-black transition-opacity hover:opacity-80"
              style={{ fontFamily:"'Barlow Condensed',sans-serif", background:'rgba(37,211,102,0.12)', border:'1px solid rgba(37,211,102,0.25)', color:'rgba(37,211,102,0.85)' }}
            >
